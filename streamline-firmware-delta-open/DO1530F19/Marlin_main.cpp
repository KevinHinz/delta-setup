...
//float max_pos[3] = { X_MAX_POS, Y_MAX_POS, Zmax };
...
void setup()
{
  ...
  if (!clayMode) tp_init();    // Initialize temperature loop
  else set_extrude_min_temp(.0);
...
void process_commands()
{
  ...
  else if(code_seen('M'))
  {
	...
	plan_buffer_line(target[X_AXIS], target[Y_AXIS], target[Z_AXIS], target[E_AXIS], feedrate/60, active_extruder);
        ...
        disable_e2();
        delay(100);
        LCD_ALERTMESSAGEPGM(MSG_PAUSE_PRINT);
        uint8_t cnt=0;
        while(!lcd_clicked()){
          cnt++;
          if (!clayMode) manage_heater();
...
if (!clayMode) {
	...
	float deltapos[4];
        ...
        deltapos[X_AXIS]=sqrt(delta_diagonal_rod_2
                       - sq(delta_tower1_x-current_position[X_AXIS])
                       - sq(delta_tower1_y-current_position[Y_AXIS])
                       ) + current_position[Z_AXIS];
        deltapos[Y_AXIS]=sqrt(delta_diagonal_rod_2
                       - sq(delta_tower2_x-current_position[X_AXIS])
                       - sq(delta_tower2_y-current_position[Y_AXIS])
                       ) + current_position[Z_AXIS];
        deltapos[Z_AXIS]=sqrt(delta_diagonal_rod_2
                       - sq(delta_tower3_x-current_position[X_AXIS])
                       - sq(delta_tower3_y-current_position[Y_AXIS])
                       ) + current_position[Z_AXIS];
        deltapos[E_AXIS]=current_position[E_AXIS];
	...
	LCD_ALERTMESSAGEPGM("-OK-");
        extFilCgn=false;
	lcd_update();
        current_position[E_AXIS]=deltapos[E_AXIS]; 
        plan_set_e_position(current_position[E_AXIS]);
	
        plan_buffer_line(delta[X_AXIS], delta[Y_AXIS], delta[Z_AXIS], deltapos[E_AXIS], feedrate/60, active_extruder); //should do nothing
        plan_buffer_line(deltapos[X_AXIS], deltapos[Y_AXIS], deltapos[Z_AXIS], deltapos[E_AXIS], feedrate/60, active_extruder); //move z back
#else
        float target[4];
        float lastpos[4];
        target[X_AXIS]=current_position[X_AXIS];
