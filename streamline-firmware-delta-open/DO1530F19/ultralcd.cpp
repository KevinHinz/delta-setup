...
static void lcd_main_menu()
{
    ...
    if (card.cardOK)
    {
		...
		MENU_ITEM(gcode, MSG_PAUSE_PRINT_WASP, PSTR("M600"));
            }//else{
...
static void lcd_prepare_menu()
{
    ...
    MENU_ITEM(submenu, MSG_WASP_MAN_LEVEL, lcd_manual_calib);   //Den aggiunta menu 
    if (!clayMode) {
#if TEMP_SENSOR_0 != 0
...
static void lcd_modify_z_max()
{
		...
		if (encoderPosition != 0)
    		{	
				refresh_cmd_timeout();
				//Zmax += float((int)encoderPosition)/2;
				Zcorrect += float((int)encoderPosition)/2;
				if (Zcorrect>3.00) Zcorrect=3.00;
		...
		if (lcdDrawUpdate)
    		{
        		//lcd_implementation_drawedit(PSTR("Z MAX ->"), ftostr32(Zmax));
        		lcd_implementation_drawedit(PSTR("Z correct "), ftostr52(Zcorrect));
    		}
    		if (LCD_CLICKED)
    		{
